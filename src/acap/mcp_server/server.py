...
"""Main stdio MCP server for the ACAP reference implementation.

MCP scaffolding portable across sibling protocol repos (phala,
pratyahara-nerve, sauvidya-pace). When copying this file, change only:

  - SERVER_NAME, SERVER_VERSION, SERVER_INSTRUCTIONS
  - the `from acap.mcp_server.tools import ...` line
...
from acap.mcp_server.tools import HANDLERS, TOOL_SCHEMAS, ToolInvocationError

SERVER_NAME = "acap"
SERVER_VERSION = "0.1.0"
SERVER_INSTRUCTIONS = (
    "ACAP (Agent Consent and Adherence Protocol) reference server. "
    "Exposes Core validators (consent chain, adherence trail, policy "
    "hash) and one primary entry point per extension (governance "
    "tiering, category preferences, regulatory context, audit "
    "projection). All tools take and return JSON. See the project "
    "README for schemas."
)
...
def main() -> None:
    """Entry point for ``python -m acap.mcp_server`` and the ``acap-mcp`` script.
